...
// DefaultReturnValue is the json structure that luet expects as output when the plugin is done
var DefaultReturnValue = map[string]string{
	"data":  "",
	"error": "",
	"state": "",
}
...
func (event LuetEvent) Run() (map[string]string, error) {
	...
	case bus.EventImagePostUnPack:
		...
		if err != nil {
			...
			return helpers.WrapErrorMap(DefaultReturnValue, err)
		}
		...
		if dataTmp.Data.Image == "" || dataTmp.Data.Dest == "" {
			...
			return helpers.WrapErrorMap(DefaultReturnValue, errors.New("fields missing from payload"))
		}
		...
		for _, s := range ImageBlacklist {
			if strings.Contains(dataTmp.Data.Image, s) {
				...
				return helpers.WrapErrorMap(DefaultReturnValue, nil)
			}
		}
